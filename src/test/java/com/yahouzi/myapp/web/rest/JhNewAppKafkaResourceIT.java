...
import com.yahouzi.myapp.config.KafkaProperties;
...
class JhNewAppKafkaResourceIT {
    ...
    void setup() {
        ...
        JhNewAppKafkaResource kafkaResource = new JhNewAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jh-new-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jh-new-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
